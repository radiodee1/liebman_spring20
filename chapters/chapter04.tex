...
There are only three Raspberry Pi boards and a Jetson Nano board. %Installation of all four models is attempted on Pi boards, but only 
...
This list of questions was asked of all models. %From this list and the answers from each model a comparison between the models about their strengths is made.
...
This model does not use a Transformer architecture. %This second sequence-to-sequence model from the GRU tutorial performed exactly as expected. 
It answered a number of subjective questions appropriately. %The answers made sense in the English language.
...
Using the Persona Corpus a Transformer model was trained, but not pre-trained, and did not use transfer learning. %The Persona Corpus comes from Mazar{\'{e}} et al \cite{DBLP:journals/corr/abs-1809-01984}.
...
The memory footprint, while running below one Gigabyte, could be installed on a Raspberry Pi board. %That requires a python package called `tensorflow-model-server' and this package had to be provided for ARMv7 for the Raspberry Pi. 

\iffalse
\subsection{Training}
First the model was trained on the Persona Corpus until a pattern emerged. When the model began to answer all questions with the phrase `I don't know' training was stopped. 
...
%When it comes upon a question that it doesn't expect it defaults to a certain sentence. 
%It can answer questions that you might ask in a rudimentary conversation. 
...
%If you tell it your name it will tell you that its name is `Sarah'. 
...
\begin{itemize}
	...
	\item []Some of the answers are re-used and do not follow logically from the questions. The `favorite color' and `favorite candy' questions prompt the model's default answer `Hi, how are you today?' % This seems to be the model's default answer.
...
This Transformer model was tested on the Raspberry Pi 4B. %The model is not more dynamic than the GRU tutorial model. 
...
%The Persona corpus model is not interesting to watch. 
...
The model took seven days to train on the development computer in a x86\_64 environment with a CPU based processor. 
...
The Transformer model is installed on the Raspberry Pi. It takes two minutes to boot and 6 seconds to answer any question. %After that the time between responses is slow. 

The Raspberry Pi gives a tone at the end of loading the model to signal the model is done processing input. % during long response times. 
...
Subjectively the model was the best of those tested. Usually the model would answer questions about it's location, it's name, and the time, correctly. %Interestingly there where times when it did not do so. 
Incorrect answers did address the particular question.
Some times it used alternative answers. For example, it would answer with the time but not the correct time or the city but not the correct city.  %This was odd.

The output was always sensible English. %There were no times where the model replied with gibberish. 
...
%The model did not remember its own answers but it was consistent. 
%Questions it answered include `What is your favorite color?' and `Do you like lollipops?' 
...
\begin{itemize}
	...
	%Some of the time the answer includes a word from the question sentence that would lead you to believe that this model has fewer stock answers. 
...
%The GPT2 model fit on the Raspberry Pi. 
...
%every time the model was processing input. 
Without this it would have been difficult to know when to speak.% and when to wait for a response. 
%Aesthetically this arrangement is not perfect, but it allows the Generative Pre-Training 2 model to be physically installed on the Raspberry Pi.

Also LED lights were installed to indicate when the model is processing input and when the model can take new input. %The lights are helpful.
