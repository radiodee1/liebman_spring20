...
It is useful to observe the model's progress during training. The Movie Dialog corpus is used here. This training is not interrupted. The section below describes how the model's output evolves.

Fist the model produces random tokens as output. Then the model begins to recognise the patterns of the input data. It begins to put an `end-of-line', or EOL, token at the end of each output. At the very beginning it is not uncommon to see output patterns that consist of nothing more than the 'end-of-line' token. 

At the next stage the model passes a barrier of sorts and will not say anything that is not proper English. 

Then the model chooses its first true sentence. This sentence is usually negative. It is usually something like `I don't know' or `I'm sorry.' It uses this sentence repeatedly for a long time. It has, at this point, no other response.

Then the model chooses a second true sentence. This sentence is usually negative also. It uses these two sentences for a long time.
