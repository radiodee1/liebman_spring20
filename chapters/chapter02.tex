...
\begin{figure}[H]
	\begin{center}
		...
		\includegraphics[scale=1.5]{diagram-mask01}
	\end{center}
	\caption[Transformer Mask]{Transformer Mask. - Decoder uses masked input during training.}
	
	
\end{figure}

During training the decoder section is masked. The mask prohibits the decoder from seeing parts of the target that it should not during training. This mimics the inference setup and still allows for large input matrices.

During inference the decoder produces a token and then it adds to that token, one at a time, until the decoding is finished and something like English is produced. It can attend to any part of the output it has already produced. During training the decoder is exposed to the target sequence under a mask.
