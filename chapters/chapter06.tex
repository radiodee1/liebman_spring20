...
Time complexity for the GRU Recurrent Neural Network is $ n \cdot d^2 $, where $ n $ is the number of words in a sentence and $ d $ is the dimension of the hidden vector. Complexity for the Transformer's Scaled Dot Product Attention is $ n^2 \cdot  d $. Again, $n$ is the number of words and $d$ is the hidden vector's dimension. In the GRU the $n$ is always set at the maximum sentence length. For the Transformer, $n$ is the length of the input sentence. It need not be the maximum allowable input.
