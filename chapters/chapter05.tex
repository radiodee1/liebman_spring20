...
For us to use AIML for our task we code questions and answers in pairs for all possible phrasings of a question.
...
User name is the first example. Using `*' as a wildcard, there are few ways to say `My name is *.' Another variant is `* is my name'. %That's as many ways to say it as there are.
...
With the smaller GPT2 model the reply is often `Jane.' %even with the AIML kernel. 
...
When the user asks `How many members were in the band?' the model seems to be able to intuit the specific band. % that you are referring to.
