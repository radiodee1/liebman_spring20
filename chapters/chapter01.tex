...
Here `$ s_t $' is the symbol for the combined output. The two inputs are `$ x_t $' and `$ s_{t-1} $' . `$ x_t $' is the regular input. `$ s_{t-1} $' is the hidden state input for the Recurrent Neural Network or Gated Recurrent Unit. Sigmoid activation is used on the two gates, using the symbol `$ \sigma(...) $' , while tanh activation is used to compute the hidden output. %The output is found in the fourth line, denoted as `$s_t$'. This serves as both hidden and regular output.
...
\begin{figure}[H]
	...
	\includegraphics[scale=0.5]{diagram-nmt02}
		
\end{center}
	\caption[Sequence to Sequence Architecture]{Seq2seq: A, B and C represent an input sequence and W, X, Y and Z represent the corresponding output. (Sutskever et al \cite{DBLP:journals/corr/SutskeverVL14})}
